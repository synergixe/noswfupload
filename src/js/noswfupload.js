...
var $ = {
            ...
            for(var max = false, input = wrap.dom.input.cloneNode(true), i = 0, files = $.files(wrap.dom.input); i < files.length; i++){
                ...
                if((wrap.fileType || type) && -1 === (wrap.fileType || type).search(new RegExp("(?:\\*|image)(?:\\.|\\/)" + ext, "ig"))){
                    ...
                    $.text(
                        wrap.dom.info,
                        $.lang.errorType
                            .replace(/{fileType}/g, wrap.fileType || type)
                    ...
                    a.href = "#" + value;
                    a.alt = a.title = (size ? "[" + $.size(size) + "] " : "") + value;
                    li.className = ext;
                    $.text(a, value);
                    $.event
                        .add(a, "click", $.empty)
    ...
    sendFile:(function(toString){
        var multipart = function(boundary, name, file, type){
            ...
            length      = split.length,
            CRLF        = "\r\n",
            xhr = this.XDomainRequest ? this.XDomainRequest : (this.XMLHttpRequest ? new XMLHttpRequest : new ActiveXObject("Microsoft.XMLHTTP")),
            sendFile;
        ...
        if(xhr.upload || xhr.sendAsBinary)
            sendFile = function(handler, maxSize){
                if(-1 < maxSize && maxSize < (handler.file.fileSize || handler.file.size)){
		...
		if(('withCredentials' in xhr.constructor.prototype)){
			// if creadentials is set - preflight response header 'Access-Control-Allow-Origin' MUST never be '*'
      ...
      try{
                ...
                if(handler.file.getAsBinary){
                    ...
                    /*
                       See: https://stackoverflow.com/questions/12348216/uploading-a-file-with-xmlhttprequest-missing-boundary-in-multipart-form-data
                    */
                    ...
                    if(typeof FileReader === 'function'){
                        ...
                        if(/^(application\/json|text\/plain)/.test(handler.file.fileName || handler.file.name)){
                            reader.readAsText(handler.file);
                        }else{
                            reader.readAsBinaryString(handler.file);
                        }
                    }else{
    ...
    sendFiles:function(handler, maxSize){
        ...
        while(handler.current < length){
            ...
            handler.total += (handler.files[current].fileSize || handler.files[current].size);
            ++handler.current;
        }
        handler.current = 0;
        
        if(length && (handler.files[0].fileSize || handler.files[0].size) !== -1){
